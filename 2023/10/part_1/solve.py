...
class Node:
    ...
    def __eq__(self, other):
        if isinstance(other, Node):
            return self.char == other.char and self.row == other.row and self.column == other.column
        return False
...
def add_node(node: Node, char: str, row: int, column: int, all_nodes):
    ...
    if node.next is None or node.next == target_node:
        ...
        print(f'Error: edges for node {node.char} at index {node.row},{node.column} already set.')
...
def solve(lines: str) -> None:
    ...
    print(find_max_distance(starting_node))
