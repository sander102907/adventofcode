...
class Hand:
    ...
    def type(self) -> Type:
        ...
        card_counts = sorted(list(Counter(self.cards.replace("J", "")).values()))

        if len(card_counts) == 0:
            card_counts = [0]

        if card_counts[-1] + jokers == 5:
            return Type.FIVE_OA_KIND
        if card_counts[-1] + jokers == 4:
            return Type.FOUR_OA_KIND
        if card_counts[-1] + jokers == 3 and card_counts[-2] == 2:
            return Type.FULL_HOUSE
        if card_counts[-1] + jokers == 3:
            return Type.THREE_OA_KIND
        if card_counts[-1] + jokers == 2 and card_counts[-2] == 2:
            return Type.TWO_PAIR
        if card_counts[-1] + jokers == 2:
